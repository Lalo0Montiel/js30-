let products = [
  ...
  */
      ...
      /*2*/ 
      const listaDescuento = array => {
        let newArray = array.map(producto => {
            let descuento = producto.price * producto.discount;
            let descuentoAplicado = producto.price - descuento
            return producto.name + ", y su precio con descuento es de $"+ descuentoAplicado;
        })
        console.log(newArray)
      }

/*3*/      listaDescuento(products)
 ...
 /*1*/    const stockBajo = array => {
        let arrayStock = array.filter(producto => {
